...
class Remote(object):
    ...
    def _sftp_get_file(self, args, file_path):
        """
        Run args passed (if applicable), then use the 
    ...
    def get_file(self, path, sudo=False): 
        ...
        """
        if not sudo:
            return self._sftp_get_file(False, path)
        ...
        return self._sftp_get_file(args, temp_file_path)
